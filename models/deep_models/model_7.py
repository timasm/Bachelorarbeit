...
class Autoencoder_Deep_7(nn.Module):
    def __init__(self):
        super(Autoencoder_Deep_7, self).__init__()
        ...
        self.encoder_conv_14 = nn.Conv2d(
            128, 128, kernel_size=3, padding=1, padding_mode='replicate')
        self.encoder_relu_14 = nn.ReLU()
        self.encoder_maxpool_7 = nn.MaxPool2d(kernel_size=2, padding=0)

        self.encoder_conv_15 = nn.Conv2d(
            128, 256, kernel_size=3, padding=1, padding_mode='replicate')
        self.encoder_relu_15 = nn.ReLU()
        ...
        self.decoder_upsample_7 = nn.Upsample(scale_factor=2, mode='bicubic')
        self.decoder_conv_13 = nn.Conv2d(
            64, 64, kernel_size=3, padding=1, padding_mode='replicate')
        self.decoder_relu_13 = nn.ReLU()
        self.decoder_conv_14 = nn.Conv2d(
            64, 64, kernel_size=3, padding=1, padding_mode='replicate')
        self.decoder_relu_14 = nn.ReLU()

        self.decoder_conv_15 = nn.Conv2d(
            64, 3, kernel_size=3, padding=1, padding_mode='replicate')
        self.decoder_relu_15 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        add = torch.add(d12, e4)
        x = self.decoder_upsample_7(add)
        x = self.decoder_relu_13(self.decoder_conv_13(x))
        d14 = self.decoder_relu_14(self.decoder_conv_14(x))
        add = torch.add(d14, e2)
        decoder = self.decoder_relu_15(self.decoder_conv_15(add))
