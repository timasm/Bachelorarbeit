...
class Autoencoder_Wide_128(nn.Module):
    def __init__(self):
        super(Autoencoder_Wide_128, self).__init__()

        # ------- Encoder ------- #
        self.encoder_conv_1 = nn.Conv2d(
            3, 32, kernel_size=3, padding=1, padding_mode='replicate')
        self.encoder_relu_1 = nn.ReLU()
        self.encoder_conv_2 = nn.Conv2d(
            32, 32, kernel_size=3, padding=1, padding_mode='replicate')
        self.encoder_relu_2 = nn.ReLU()
        self.encoder_maxpool_1 = nn.MaxPool2d(kernel_size=2, padding=0)

        self.encoder_conv_3 = nn.Conv2d(
            32, 64, kernel_size=3, padding=1, padding_mode='replicate')
        ...
        self.decoder_conv_3 = nn.Conv2d(
            64, 32, kernel_size=3, padding=1, padding_mode='replicate')
        self.decoder_relu_3 = nn.ReLU()
        self.decoder_conv_4 = nn.Conv2d(
            32, 32, kernel_size=3, padding=1, padding_mode='replicate')
        self.decoder_relu_4 = nn.ReLU()

        self.decoder_conv_5 = nn.Conv2d(32, 3, kernel_size=3,
